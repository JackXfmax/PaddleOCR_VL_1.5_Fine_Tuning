...
with open(DP, "r", encoding="utf-8") as f:
    for idx, line in enumerate(f):
        ...
        if (idx + 1) % 20 == 0 or idx < 3:
            log("  [{}/{}sim={:.4f} em={:.1f} | {}".format(idx + 1, s, e, iname))

avs = sum(r["similarity"] for r in results) / len(results)
ave = sum(r["exact_match"] for r in results) / len(results)
log("")
log("=== EVAL RESULTS ===")
log("Total: {} samples".format(len(results)))
log("Avg NED: {:.4f}".format(avs))
em_count = int(sum(r["exact_match"] for r in results))
log("Exact Match: {:.2f} ({}/{})".format(ave, em_count, len(results)))
