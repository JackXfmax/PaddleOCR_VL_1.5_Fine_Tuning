...
Deployed on old server (222.19.225.132) with GPU support
Port: 8899
"""
import os
import sys
...
MODEL_PATH = "/home/xufei/tibetan_ocr_lora/stage2_manual_20260406/exp_B_joint_full_manual_20260406/export"
IMAGE_ROOT = "/home/xufei/natural_scene/"
EVAL_PATH = "/home/xufei/tibetan_ocr_lora/stage2_manual_20260406/eval_B_multilingual_manual_20260406.json"
...
def load_model():
    ...
    print("[INFO] Using GPU:0", flush=True)
    ...
    print("[INFO] Model ready on GPU!", flush=True)
...
def ocr():
    ...
    try:
        ...
        return jsonify({"text": result, "time_sec": elapsed, "model": "PaddleOCR-VL-1.5 + LoRA (B_manual)"})
...
def demo_samples():
    try:
        ...
        top = sorted(results, key=lambda x: -x["normalized_similarity"])[:12]
        samples = []
        for r in top:
            ...
            if os.path.exists(img_path):
                ...
                samples.append({
                    ...
                    "ground_truth": r["gt"],
                    "prediction": r["pred"],
                    "similarity": round(r["normalized_similarity"], 4)
                })
        return jsonify({
            "avg_similarity": round(d.get("avg_normalized_similarity", 0), 4),
            "exact_match_rate": d.get("exact_match_rate", 0),
            "total": len(results),
            "samples": samples
        })
    except Exception as e:
