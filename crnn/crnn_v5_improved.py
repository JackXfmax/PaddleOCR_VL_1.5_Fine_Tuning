...
class TibetanDataset(Dataset):
    def __init__(self, data_dir, split='train', char_to_idx=None, max_length=200):
        ...
        print(f'{split} set: {len(self.image_files)} samples (filtered from {len(all_images)})')
    ...
    def __getitem__(self, idx):
        ...
        # 自适应resize：保持高度32，宽度按原始比例，最大1600
        w, h = image.size
        aspect = w / h
...
class CRNN(nn.Module):
    def __init__(self, num_classes):
        ...
        # 更强的RNN: 3层双向LSTM + 更大的隐藏层
        self.rnn = nn.LSTM(1024, 256, 3,  # 3层
                          bidirectional=True, 
                          batch_first=True, 
                          dropout=0.3)
...
def train():
    ...
    train_dataset = TibetanDataset('/home/xufei/tibet_acent', split='train', max_length=200)
    ...
    os.makedirs('/home/xufei/tibet_acent/checkpoints_v5', exist_ok=True)
    ...
    for epoch in range(150):  # 更多轮次
        model.train()
        total_loss = 0
        
        pbar = tqdm(train_loader, desc=f'Epoch {epoch+1}/150')
        ...
        if char_acc > best_char_acc:
            ...
            }, '/home/xufei/tibet_acent/checkpoints_v5/best.pth')
            print(f'  -> New best model saved! CharAcc={char_acc:.4f}')
        else:
            no_improve += 1
        ...
        if (epoch + 1) % 10 == 0:
            torch.save({
                ...
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
            }, f'/home/xufei/tibet_acent/checkpoints_v5/epoch_{epoch+1}.pth')
    
    with open('/home/xufei/tibet_acent/checkpoints_v5/history.json', 'w') as f:
